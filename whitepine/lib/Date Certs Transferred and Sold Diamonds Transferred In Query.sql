...
CASE WHEN dbo.Inventory.WorkCenter IS NULL THEN dbo.InvoiceHeader.InvDate ELSE NULL END as DateSold,
datediff(d, dbo.Transactions.TransDate, ISNULL(CASE WHEN dbo.Inventory.WorkCenter IS NULL THEN dbo.InvoiceHeader.InvDate ELSE NULL END, GETDATE())) as "DaysToSell/CurrentAge",
CASE WHEN dbo.Inventory.WorkCenter IS NULL THEN dbo.InvoiceDetail.SellPrice ELSE NULL END as SellPrice,
CASE WHEN dbo.Inventory.WorkCenter IS NULL THEN dbo.tblCustomers.CompanyName ELSE NULL END as Customer,
dbo.Inventory.CurrentValue as CostForInventoryInStock,
